function [tint, yint] = GeneTherapySystemElimination(tspan, vars)
...
function dx=f1(t, x, T1, T2, Tg, Dtfr, Dtfp, Dg, n, Ka, Tcr, Tcp, Dcr, ...
        Dcp, Kcg, k1, Km1)
    dx=[x(9)*T1-x(1)*Dtfr; % TF % Eqn 1
        x(1)*T2-x(2)*Dtfp; % TF % Eqn 2
        x(11)*Tg*(1-x(2)^n/((Ka*(x(11)+x(12)))^n+x(2)^n))-x(3)*x(6)*Kcg-x(3)*Dg; % gRNA1
        x(12)*Tg*(1-x(2)^n/((Ka*(x(11)+x(12)))^n+x(2)^n))-x(4)*x(6)*Kcg-x(4)*Dg; % gRNA2 % I think this corresponds to eqn 12 in Dash's draft
        x(10)*Tcr-x(5)*Dcr; % Cas9 % Equation 4
        ...
        -k1*x(7)*x(12)/(Km1+x(9)+x(10)+x(11)+x(12)+x(15)) % gRNA2 % Not sure what this is, one of the n's in the paper
