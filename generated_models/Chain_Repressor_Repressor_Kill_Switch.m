function [time_interval, y_out, y] = Chain_Repressor_Repressor_Kill_Switch(time_span, parameters, initial, step)
...
function dx=diff_eq(t, x, parameters)
	...
	d_TF2 =  alpha_p_TF2*(K_R^n)/(K_R^n + TF^n)*AAV - delta_TF2*TF2;
	d_Cas9 =  alpha_p_Cas9*AAV - Cas_gRNA_binding*Cas9*sgRNA1 - Cas_gRNA_binding*Cas9*sgRNA2 - delta_Cas9*Cas9;
	d_sgRNA1 =  alpha_r_sgRNA1*(K_R^n)/(K_R^n + TF2^n)*AAV - Cas_gRNA_binding*Cas9*sgRNA1 - delta_g*sgRNA1;
    ...
    dx = real([d_AAV, d_Cas9, d_Cas9_sgRNA1, d_Cas9_sgRNA2, d_TF, d_TF2, d_edited_genome, d_genome, d_postedit_Cas9_sgRNA1, d_postedit_Cas9_sgRNA2, d_sgRNA1, d_sgRNA2])';
end
