function [time_interval, y_out, y] = Joint_Activator_Activator_Kill_Switch(time_span, parameters, initial, step)
...
function dx=diff_eq(t, x, parameters)
	...
	d_sgRNA1 =  alpha_r_sgRNA1*(TF^n)/(K_A^n + TF^n)*(TF2^n)/(K_A^n + TF2^n)*AAV - Cas_gRNA_binding*Cas9*sgRNA1 - delta_g*sgRNA1;
