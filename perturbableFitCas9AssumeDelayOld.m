function hourlyLag = perturbableFitCas9AssumeDelayOld(lag, Tc, T1)
    for i = 1:14
%     for i = 1:336
%         c(i) = i-0.99;
        c(i)=i*24-23.99;%c(1) has to be larger than 0, so......
        tspan=[0,c(i)];
        options = [];
        ...
        d(i)=100*sol.y(5,end)/(sol.y(4,end)+sol.y(5,end));% the way that next generation sequencing detect mutation rate.
    end
    hourlyLag = [c; d]';
