    ...
    for k = 1:length(whichVars)
        if whichVars(k)
            ...
            for l = 1:length(percentiles)
                logDist(l) = 10^(randn()*log10(stddev)+log10(mean));
            end
            varsToUse(:, k) = logDist;
        else
...
%     results(i, 1) = {percentiles(i)};
    ...
    results(i, 2) = {perturbableFitCas9AssumeDelay([0, tspan], varsToUse(i, 1), varsToUse(i, 2), varsToUse(i, 3))};
...
save('20210330-rand-lag.mat', 'logDist', 'results');